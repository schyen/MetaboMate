...
spec.quality=function(X, ppm, ppm.noise=c(9.4,9.5), plot=T){
  ...
  if(min(ppm.noise)<min(ppm) | max(ppm.noise)>max(ppm)){ stop('Chemical shift specified in ppm.noise variable is not included in ppm vector (out of range).')}
  ...
  if(min(ppm)>-0.01 | max(ppm)<9.5){stop('A minimum spectral range of -0.01 tp 9.5 ppm is required.')}
  # TSP line width estimataion
  tsp.lw=lw(X, ppm, shift=c(-0.01,0.01))

  # residual water
  idx=get.idx(range=c(4.7, 4.95), ppm)
  if(length(idx)<10){stop('It looks like the water signal has been removed. A minimum spectral range of -0.01 tp 9.5 ppm is required.')}
