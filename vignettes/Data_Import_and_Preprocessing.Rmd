...
For illustration purposes we plot the TSP signal (a component of the sample preparation buffer) coloured accroding to the receiver gain paramter in **meta** dataframe. The RG value amplifies the NMR signal (free induction decay) before it is converted to a digital signal. 
...
From the plot above you can see that there were different types of experiments performed (indicated by different linetypes) and that the RG is related to the TSP signal strength within each experiment type. **Meta** also contains information on sample acquisition date and time (**a_Date**) and run order (**a_RunOrder**).


The different experiment types map to different biolfuids and NMR pulse sequences, making a quantitative comparison not useful and therefore, we filter for the desired experiment type (**\<JL-noe1d\>**) before we continue with the analysis.

```{r, fig.show='hold', fig.width = 7.2, fig.height = 4}
# plot TSP signal

idx=grep('noe', meta$a_EXP)
X=X[idx,]
meta=meta[idx,]
...
We can calibrate the urine spectra using the **calibrate()** function, as illustrated with the following code:
```{r, fig.show='hold', fig.width = 7.2, fig.height = 4}
...
High throughput NMR often does not allow a manual inspection of each individual spectrum and automatically generated quality indices are often used to exclude low quality spectra. Here, we derive a table of quality indices where an overview plot is optional:
...
Currently, the line width estimate of the **spec.quality()** function requires a TSP signal or any other external reference compound that resonates at zero ppm. 

## Excision of chemical shifts regions and baseline correction
 
Further processing such as spectral normalistion (next section) requires the removal of non-relevant chemical shift regions. In urinary NMR analyses these usually include the TSP and residual water resonances as well as ppm regions where there is not signal but just noise.

The function **get.idx()** can be used to obtain indices of the ppm vector with desired shift range. These can then be used to exclude the relevant columns in the NMR matrix and ppm vector, as illustrated in the following code snippet.
...
Baseline correction can be performed with the function **bline()**. See **?bline** for further information.
