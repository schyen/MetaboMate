...
This tutorial runs through a typical data import and pre-processing workflow for Nuclear Magnetic Resonance (NMR) - based metabolic profilig with the *MetaboMate* R package. First of all, load the *MetaboMate* package:
...
For illustration purposes let's plot the TSP signal, which is a reference compound located in the ppm range of -0.05 to 0.05. Added is information on acquisition paramters that can be found in the *meta* dataframe: the different panels (facetting) show different experiment types, the colour maps to the receiver gain (*a_RG*) and the linetype indicates the NMR pulse program (*a_PULPROG*):
...
The plot above shows that there were three different experiment types performed (i.e., three different panels), all with pulse program *<noesypr1d>* which a Bruker term for a standard 90˚ pulse experiment with water pre-saturation. The RG value should really be constant or in a small and defined range, which is the case for the respective experiment type. 



## Spectral referencing

Spectral referencing refers to horizontally shifting an entire spectrum unitl a specific signal reaches a defined chemical shift. 
In urine metabolic profiling, the standard compound  is TSP^[3-(trimethylsilyl)-2,2′,3,3′-tetradeuteropropionic acid] (usually added to the sample buffer), which gives rise to a singlet that is defined to resonate at 0 ppm.^[Dona, A.C., *et al.* (2014) Precision high-throughput proton NMR spectroscopy of human urine, serum, and plasma for large-scale metabolic phenotyping. *Analytical Chemistry*. 86.19. 9887-94.]
...
Now you can see that for all spectra, the TSP peak apex is centered at chemical shift of zero ppm (that was not the case before). 
...
For statistical analysis we are only interested in experiment type *<JL-noe1d>* as all other ones are calibration experiments. To exclude the latter, execute the following R code which also re-runs the previous plotting command:
...
Both of the experiment types labelled **\<\>** and **\<OB_flowtest\>** (upper and lower panel in the above plot) are calibration experiments and were performed at the beginning of the run (as indicated by the colour). For quantitative analysis these are not suitable and therefore, the desired experiment type (**\<JL-noe1d\>**) will be filtered before we continue with the analysis.
