...
Plotted is the covariance of PCA scores (*t*) and NMR variables (*x*) over ppm. The colour represents Pearson's correlation coefficient of *t* and *x*. Signals coloured towards the red colour spectrum are characteristic for spectra with postive (negative) scores when these point up (down) in the loadings plot.
...
The resuling OPLS CV scores plot shows an almost perfect separation of both surgery types, with all predictive power focussed on the x-axis (that is characteristic for O-PLS, compared to PLS). 
