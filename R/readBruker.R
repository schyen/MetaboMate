...
readBruker <- function(path, filter = T) {
    ...
    afile <- list.files(path = path, pattern = "^acqus$", all.files = FALSE, full.names = TRUE, recursive = TRUE, ignore.case = TRUE)
    pfile <- list.files(path = path, pattern = "^procs$", all.files = FALSE, full.names = TRUE, recursive = TRUE, ignore.case = TRUE)
    pfile <- grep("pdata/1/", pfile, value = T)
    rfile <- list.files(path = path, pattern = "^1r$", all.files = FALSE, full.names = TRUE, recursive = TRUE, ignore.case = TRUE)
    ...
    out <- apply(rbind(1:Lp), 2, function(i, pf = pfile, rf = rfile, af = afile) {
        ...
        if (as.numeric(meta$V2[grep("p_BYTORDP", meta$V1)]) != 0) {
            endianness <- "big"
        } else {
            endianness <- "little"
        }
        spec <- readBin(rf[i], what = "int", n = as.numeric(meta$V2[grep("p_FTSIZE", meta$V1)]), size = 4, signed = T, endian = endianness)
        spec <- ((2^as.numeric(meta$V2[grep("p_NC_proc", meta$V1)])) * spec)
        swp <- as.numeric(meta$V2[grep("p_SW_p", meta$V1)])/as.numeric(meta$V2[grep("^p_SF$", meta$V1)])
    ...
    # The offset is slightly different in each exp, leading to NA values for few ppm variables at the ends of some spectra. Remove these if not too
    # many!
