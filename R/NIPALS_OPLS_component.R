...
NIPALS_OPLS_component <- function(X, Y) {
    ...
    while (dd > 1e-10) {
        ...
        # if(count>1){ dd=sum((t_h[,1]-t_old[,1])^2); #print(dd) } 4 calc loadings Y -> c is q
        c_h <- (t(t_h) %*% Y)/drop(crossprod(t_h))
        # c_h=c_h/sqrt(sum(c_h[1,]^2)) # normalisation 5. cal new u and compare with one in previus iteration (stop criterion)
        u_new <- (Y %*% t(c_h))/drop(crossprod(t(c_h)))
        if (count > 1) {
            dd <- sum((u_new - u)^2)/sum(as.numeric(u_new)^2)
            print(dd)
        }
        u <- u_new
        count <- count + 1
    ...
    # # for multicolumn Y: estimate orthogonal component # 11: orthogonalise p_h (use the scores of Y weights: T_w) for(i in 1:ncol(T_w)){
    # t_w=cbind(T_w[,i]) p_h=p_h - ((t(t_w) %*% t(p_h)/crossprod(t_w)[1,1]) %*% t(t_w)) } step 7
    ...
    # 16: return paramters 11: Filter data
    E_opls <- X - (t_o %*% p_o)
    # Y_res = Y - u %*% c_h
